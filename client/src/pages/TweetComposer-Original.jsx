...
const TweetComposer = () => {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (error.response?.data?.code === 'TWITTER_RATE_LIMIT') {
        toast.error('⏰ Twitter rate limit reached. Please try again in 15-30 minutes. Your credits have been refunded.', { duration: 8000 });
        return;
      }
      ...
      if (error.response?.data?.code === 'TWITTER_PERMISSIONS_ERROR') {
        toast.error('Twitter permissions expired. Please reconnect your Twitter account.', { duration: 6000 });
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.details || error.response?.data?.error || 'Failed to generate content';
    ...
    try {
      ...
      if (response.data && response.data.success && response.data.imageUrl) {
        ...
        if (imageSize > MAX_IMAGE_SIZE) {
          toast.error(`Generated image is too large (${(imageSize / (1024 * 1024)).toFixed(1)}MB). Max allowed is 5MB.`);
          return;
        }
  ...
  const handleImageButtonClick = () => {
    ...
    if (showImagePrompt) setImagePrompt('');
  };
  ...
  return (
    <>
      {/* Main content with right margin to avoid overlap with side panel */}
      <div className="max-w-2xl mx-auto space-y-6 lg:mr-64">
        {/* Header */}
        <div>
          ...
          <div className="space-y-4">
            ...
            {isThread && (
              <div className="space-y-3">
                ...
                <button onClick={addThreadTweet} className="btn btn-secondary btn-sm">
                ...
                <div className="border border-purple-200 rounded-lg p-4 bg-purple-50">
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      onKeyPress={(e) => {
                        if (e.key === 'Enter' && !isGeneratingImage) handleImageGenerate();
                      }}
                    />
                    <button
      ...
      {/* ─── Right-side "Post to" panel ─── */}
      <div className="fixed right-6 top-1/2 -translate-y-1/2 hidden lg:block z-10">
        <div className="w-56 bg-white border border-gray-200 rounded-xl shadow-lg p-4 space-y-3">
          {/* Header */}
          ...
          {/* LinkedIn row */}
          <div className="flex items-center justify-between gap-3 py-2 px-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
            <div className="flex items-center gap-2 min-w-0">
              {/* LinkedIn "in" logo */}
              <div className="h-8 w-8 flex-shrink-0 rounded-md bg-[#0A66C2] text-white flex items-center justify-center font-bold text-base leading-none select-none">
                in
              </div>
              <div className="min-w-0">
                <div className="text-sm font-semibold text-gray-800 leading-tight">LinkedIn</div>
            ...
            <label className="relative inline-flex items-center cursor-pointer flex-shrink-0">
              ...
              />
              <div className="w-10 h-6 bg-gray-200 rounded-full peer peer-checked:bg-blue-600 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-4 shadow-inner" />
            </label>          </div>

          {/* Status hint */}
          <p className="text-xs text-gray-400 text-center">
            {postToLinkedin ? '✓ Will also post to LinkedIn' : 'Toggle to cross-post'}
          </p>
        </div>
      </div>
    </>
